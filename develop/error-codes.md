...
As APIs do Centro de Parceiros devolvem um objeto JSON que contém um código de estado. Este código indica se o seu pedido foi bem sucedido ou por que falhou.
...
| Não É Implacável      | 501       | 0          | Exceção não definida       |
...
| BadRequest          | 400       | 2012       | Incapaz de converter entre Commerce e Azure Ative Directory IDs de subscrição.          |
...
| BadRequest          | 400       | 2100       | A Oferta API não suporta item com {0} ID. Experimente utilizar os Produtos e/ou APIs SKUS      |
...
| BadRequest          | 400       | 2054       | O ciclo de faturação de uma subscrição existente não pode ser atualizado através de uma operação de subscrição de atualização. Utilize a operação de actualização-Encomenda                |
| BadRequest          | 400       | 2055       | Etag já não é válido     |
| BadRequest          | 400       | 2071       | MPN ID não é um conselheiro válido     |
| NotFound      | 404       | 2072       | ID MPN de Fornecedor Indireto não é válido como Parceiro Em Registo          |
| Proibido     | 403       | 2085       | A oferta não é elegível para compra               |
| BadRequest          | 400       | 4018       | Incapaz de processar esta ordem      |
| BadRequest          | 400       | 6000       | O ciclo de faturação não podia ser alterado porque a encomenda contém uma ou mais subscrições que não estão ativas         |
| BadRequest          | 400       | 6001       | O ciclo de faturação não podia ser alterado porque um dos IDs de oferta da encomenda não suporta o termo de faturação     |
| Proibido     | 403       | 13605      | Ou uma confirmação do parceiro da aceitação do cliente do Acordo de Cliente da Microsoft deve ser fornecida ou o cliente deve aceitar o Acordo de Cliente da Microsoft no Microsoft Admin Center antes de concluir esta compra           |
| NotFound      | 404       | 20 000      | ID de encomenda não encontrado          |
| BadRequest          | 400       | 27006      | Limite de utilização é ultrapassado para ID de oferta      |
...
| BadRequest          | 400       | 600049     | Falta informação sobre o registo da organização <br/><br/>Este erro ocorre se a empresa/organização do cliente estiver localizada num dos seguintes países e a organizaçãoRegistrationNumber não tiver sido fornecida. Países afetados:<br/><br/>- Arménia (AM) <br/>- Azerbaijão (AZ)<br/>- Bielorrússia (BY)<br/>- Hungria (HU)<br/>- Cazaquistão (KZ)<br/>- Quirguistão (KG)<br/>- Moldávia (MD)<br/>- Rússia (RU)<br/>- Tajiquistão (TJ)<br/>- Uzbequistão (UZ)<br/>- Ucrânia (UA)       |
| BadRequest          | 400       | 600050     | As informações de ID de registo da organização são inválidas e devem ser do formato \{ 0\} <br/><br/>Este erro ocorre se a organizaçãoRegistrationNumber não for válida para o país localizado da empresa do cliente. \{0 \} terá o formato de expressão regular esperado do erro. Por exemplo: `Organization registration ID information is invalid and should be of the format ^(\\d{7}|\\d{10})$`.          |
...
| Não autorizado        | 401       | 800001     | Sócio Token desaparecido no contexto do pedido             |
| BadRequest          | 400       | 800002     | Entrada de pedido inválido       |
| InternalServerError | 500       | 800003     | Erro de serviço inesperado          |
| BadRequest          | 400       | 800004     | ID de oferta inválida      |
...
| BadRequest          | 400       | 800011     | Esta subscrição não é uma subscrição válida da Azure        |
...
| Proibido     | 403       | 800023     | Não é elegível para esta transação porque a sua Linha de Crédito não está a atingir o limiar mínimo para esta compra. Atualizar a sua encomenda (ou) contactar Suporte para ajuda       |
| BadRequest          | 400       | 800024     | Você não é elegível para esta transação            |
| BadRequest          | 400       | 800025     | Não é elegível para esta transação porque a sua Linha de Crédito não está a atingir o limiar mínimo para esta compra. Atualizar a sua encomenda (ou) contactar Suporte para ajuda       |
...
| Proibido     | 403       | 800069     | Verificação falhada de Gestão de Risco falhada      |
| BadRequest          | 400       | 800070     | A oferta não está disponível no país especificado      |
| BadRequest          | 400       | 800071     | Número de item de linha deve ser de 0 para número de itens de linha conta        |
| BadRequest          | 400       | 800071     | Número de item de linha deve ser de 0 para número de itens de linha conta        |
...
| Proibido     | 403       | 800077     | O cancelamento do SaaS baseado em assentos não é suportado por ordem de remendos. Em vez disso, utilize a subscrição de patch.       |
...
| NotFound      | 404       | 800111     | Não é encontrada a subscrição Azure com o ID de direito dado.       |
| Proibido     | 403       | 800115     | Overage já está atribuído a outro inquilino. Contacte o seu cliente para resolver questões de propriedade.       |
| Proibido     | 403       | 800114     | Não é elegível para gerir a sobreatenção deste cliente.       |
| Proibido     | 403       | 800112     | O overage não pode ser definido, uma vez que o cliente tem subscrições Azure legados.       |
...
| BadRequest          | 400       | 900111     | A iD de atualização fornecida não pertence ao {0} cliente. O cliente está mapeado para atualizar o ID {1}     |
...
| Proibido     | 403       | 900115     | O plano Azure não pode ser transferido para estado suspenso uma vez que tem uma ou mais Assinaturas Azure no estado ativo        |
...
| Proibido     | 403       | 900120     | Não pode processar a atualização do Azure      |
| Proibido     | 403       | 900121     | Incapaz de processar a ordem devido a um limite de crédito insuficiente, contacte ucmwrcsp@microsoft.com para assistência adicional        |
...
| BadRequest          | 400       | 900132     | Este pedido de transferência não pode ser iniciado uma vez que o plano Azure não está ativo, por favor, adije o plano Azure     |
...
| NotFound      | 404       | 900140     | Não é possível obter subscrições Azure Ative Directory para um cliente com ID de inquilino {0} e ID de provisão de fonte{1}     |
...
| BadRequest          | 400       | 900145     | Este pedido de transferência de propriedade de faturação não pode ser concluído, uma vez que as reservas da Azure não transferem com subscrições. Cancele as reservas Azure associadas às subscrições da sua seleção e tente novamente.         |
...
| Proibido     | 403       | 900150     | A função fornecida não tem os direitos de realizar a operação solicitada      |
| InternalServerError | 500       | 900192     | Ocorreu um erro. Por favor, tente de novo depois de algum tempo.       |
| BadRequest          | 400       | 900203     | CatalogItemId: {0} requer aceitação de atestado.        |
| BadRequest          | 400       | 600049     | Falta informação sobre o registo da organização. Este erro é lançado se a empresa/organização do cliente estiver localizada nos seguintes países: Arménia(AM), Azerbaijão(AZ), Bielorrússia(BY), Hungria(HU), Cazaquistão(KZ), Quirguistão(KG), Moldávia (MD), Rússia(RU), Tajiquistão(TJ), Uzbequistão(UZ), Ucrânia(UA), mas organizaçãoRegistrationNumber não é aprovada.      |
...
| BadRequest          | 400       | 600051     | O número de telefone do cliente desapareceu. Este erro é lançado se a empresa/organização do cliente estiver localizada nos seguintes países: Arménia(AM), Azerbaijão(AZ), Bielorrússia (BY), Hungria(HU), Cazaquistão (KZ), Quirguistão (KG), Moldávia (MD), Rússia(RU), Tajiquistão(TJ), Uzhbequistão(UZ), Ucrânia(UA) mas billingProfile.defaultAd.phoneNumber não é aprovado.       |
